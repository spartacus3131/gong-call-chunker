...
- API calls go through the Next.js API proxy route at `web/src/app/api/v1/[...path]/route.ts` — NOT next.config.js rewrites (those were removed)
- The proxy decodes the next-auth session cookie, signs a short-lived HS256 JWT with user identity, and attaches it as `Authorization: Bearer <jwt>` on every backend request
- `AUTH_SECRET` / `NEXTAUTH_SECRET` must match between the web and API services — this is the shared HMAC secret
...
## Auth Architecture (as of 2026-02-27)

```
Browser
  └─ Google OAuth via next-auth (/api/auth/*)
       └─ Session cookie (authjs.session-token)
            └─ /api/v1/[...path]/route.ts (proxy)
                 ├─ getToken() decodes session cookie
                 ├─ SignJWT signs HS256 JWT (5min TTL) with sub/email/name/picture
                 ├─ Forwards to FastAPI backend with Authorization: Bearer <jwt>
                 └─ Backend api/auth.py verifies JWT using AUTH_SECRET
```

Key env vars:
- `AUTH_SECRET` (web) == `NEXTAUTH_SECRET` (api) — must match, shared HMAC secret
- `NEXT_PUBLIC_API_URL` (web) — Railway URL of the API service
- `ALLOWED_ORIGINS` (api) — must include the web service URL
...
6. **Secret mismatch breaks auth silently** — if `AUTH_SECRET` != `NEXTAUTH_SECRET`, the proxy generates a JWT but the backend rejects it with 401. Check both env vars first when debugging auth issues.
7. **No rewrites in next.config.js** — the proxy route replaced them. Do not add rewrites back or requests will skip JWT injection.
